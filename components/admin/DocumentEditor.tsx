...
import { EventLabel } from '@/lib/tiptap/EventTableExtension'
...
export default function DocumentEditor({ document, clientId, isNew = false }: Props) {
  ...
  const handleInsertToc = useCallback(() => {
    ...
    if (!toc) { alert('Brak nagłówków H2/H3 w dokumencie.'); return }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-0.5 p-1.5 bg-white border border-gray-100 rounded-lg flex-wrap">
        ...
        {/* Przycisk Event */}
        <Btn onClick={() => editor.chain().focus().setNode('eventLabel').run()} active={editor.isActive('eventLabel')} title="Oznacz jako Event GA4">
          <span className="text-xs font-bold" style={{ color: editor.isActive('eventLabel') ? 'white' : '#FF8282' }}>EV</span>
        </Btn>
        <Divider />
        ...
        {editor.isActive('table') && (
          <>
            <Btn onClick={() => editor.chain().focus().addColumnAfter().run()} title="Dodaj kolumnę"><span className="text-xs">+kol</span></Btn>
            <Btn onClick={() => editor.chain().focus().addRowAfter().run()} title="Dodaj wiersz"><span className="text-xs">+wier</span></Btn>
            <Btn onClick={() => editor.chain().focus().deleteColumn().run()} title="Usuń kolumnę"><span className="text-xs">-kol</span></Btn>
            <Btn onClick={() => editor.chain().focus().deleteRow().run()} title="Usuń wiersz"><span className="text-xs">-wier</span></Btn>
        ...
        <Btn onClick={() => editor.chain().focus().redo().run()} title="Ponów">
