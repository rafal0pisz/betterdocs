...
async function getClientWithSections(id: string) {
  ...
  const { data: sections } = await supabase
    .from('sections').select('*').eq('client_id', id).order('order_index')

  const { data: documents } = await supabase
    .from('documents').select('*').eq('client_id', id).order('order_index')

  return { client: client as Client, sections: (sections ?? []) as Section[], documents: (documents ?? []) as Document[] }
}
...
export default async function AdminClientPage({ params }: Props) {
  ...
  const data = await getClientWithSections(id)
  if (!data) notFound()
            ...
            <div key={section.id} className="bg-white border border-gray-100 rounded-xl overflow-hidden">
              ...
              {sectionDocs.length === 0 ? (
                <div className="px-5 py-4">
                  <p className="text-xs text-gray-400">Brak dokumentów w tej sekcji.</p>
                </div>
              ) : (
