...
export default async function DocumentPage({ params }: Props) {
  ...
  const [{ data: events }, { data: parameters }] = await Promise.all([
    supabase.from('structured_events').select('*').eq('document_id', docId).order('order_index'),
    supabase.from('structured_parameters').select('*').eq('document_id', docId).order('order_index'),
  ])
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 md:px-8 py-6 md:py-10">
      ...
      <div className="flex items-center gap-2 text-xs text-gray-400 mb-6 flex-wrap">
        ...
        <span>/</span>
        <Link href={`/portal/${slug}/${sectionId}`} className="hover:text-gray-600 transition-colors">{section.title}</Link>
        <span>/</span>
